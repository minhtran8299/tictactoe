...
const Greeting = props => {
  ...
  const checkGameCode = newGameCode => {
    return firestore().collection("xo-games").where("gameCode", "==", newGameCode)
      .get()
      .then(results => {
        if (results.size > 0) {
  ...
  const saveGame = newGameCode => {
    return firestore().collection("xo-games").add({
  ...
  const joinGame = () => {
    if (gameCode.length === 4) {
      return firestore().collection("xo-games").where("gameCode", "==", gameCode.toUpperCase())
      .get()
      .then(results => {
        if (results.size > 0) {
